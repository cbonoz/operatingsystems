...
MODULE_AUTHOR("Chris Buonocore, Yuanchen Liu");

#define OSPRD_MAJOR	222 //max num of possible disks to lock
...
typedef struct osprd_info {
	...
	// to understand them.
	req_queue_t *queue;    // The device request queue.
...
/*
static void free_dt(dead_list_t *t) {	
	dead_list_t *temp;
	while (t) {
		temp=t;
		t=t->next;
...
static void assert(int cond, osprd_info_t *d) {
	if (cond==0) {
...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	if (rq_data_dir(req)==READ) {
		...
		if (debug) eprintk("Process Request: Finished Read\n");

	}
	else {
		memcpy(d->data + (req->sector*SECTOR_SIZE), req->buffer,  req->current_nr_sectors*SECTOR_SIZE);
		if (debug) eprintk("Process Request: Finished Write\n");
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		{
			if(removeNode(&d->writeList, current->pid)==0)
			{
				return -EDEADLK;
			}
		}
		...
		if (filp_writable) 
		{		
			if (debug) eprintk("[%d]Attempting to acquire - write\n", cur);
			...
			//if (wait_event_interruptible(d->blockq, my_ticket==d->ticket_head && !d->writelockset && (!d->readlockset || !filp_writable)))
			{
				...
				osp_spin_lock(&d->mutex);
				if (debug) eprintk("[%d]ticket:%d being killed\n", cur, my_ticket);
				r= -ERESTARTSYS;
			...
			else 
			{
				...
				osp_spin_lock(&d->mutex);
				if (debug) eprintk("[%d]ticket:%d Acquired write lock\n", cur, my_ticket);
		...
		{
			if (debug) eprintk("[%d]Attempting to acquire - read\n", cur);
			...
			else 
			{
				...
				osp_spin_lock(&d->mutex);
				if (debug) eprintk("[%d]ticket:%d Acquired read lock\n", cur, my_ticket);
				//d->ticket_tail++;
		...
		if (debug) eprintk("[%d]Attempting to try acquire\n", cur);
		r = -ENOTTY;
		...
		if (debug) eprintk("[%d]Unlocking the ramdisk...\n", cur);
...
static void cleanup_device(osprd_info_t *d)
{
	...
	//clean up read and write lock queues
	/*
	if (d->readlockset)
		blk_cleanup_queue(d->readlockset);

	if (d->writelockset)
		blk_cleanup_queue(d->writelockset);
		*/
}
